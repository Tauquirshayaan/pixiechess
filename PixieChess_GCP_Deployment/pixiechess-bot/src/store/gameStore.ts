...
export const useGameStore = create<GameStore>()(
  ...
  updateBoardStatus: () => {
    ...
    const freshParalysis = calcBasiliskParalysis(board, colorToMove);
    const freshGameState = {
      ...gameState,
      paralyzed: { ...gameState.paralyzed, [colorToMove]: freshParalysis }
    };

    const inCheck = isCheck(board, colorToMove, freshGameState);
    const legalMoves = getAllMovesForColor(board, colorToMove, freshGameState);
    ...
    if (legalMoves.length === 0) {
      if (inCheck) {
        ...
        set({ boardStatus: 'game_over', statusWinner: null });
        return;
      }
    }
