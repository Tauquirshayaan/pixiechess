...
void generate_pseudo_legal_moves(const Board& b, MoveList& list) {
    ...
    while (frozen) {
        ...
        // Generate Unfreeze Move (skips turn to decrement freeze)
        if (get_bit(our_pieces, sq)) {
            int p_type = b.get_piece_on_square(sq);
            if (p_type != PIECE_TYPE_NONE) {
                list.add(Move(sq, sq, p_type, ICICLE, PIECE_TYPE_NONE, false, true));
            }
        }
    }
    ...
    U64 knights = (b.pieces[us][KNIGHT] | b.pieces[us][ELECTROKNIGHT] | b.pieces[us][BANKER] | 
                   b.pieces[us][KNIGHTMARE] | b.pieces[us][FISH_KNIGHT]) & ~paralyzed_squares;
    while (knights) {
    ...
    U64 kings = b.pieces[us][KING] & ~paralyzed_squares;
    while (kings) {
    ...
    while (sumorooks) {
        ...
        while (attacks) {
            ...
            if (er >= 0 && er < 8 && ec >= 0 && ec < 8 && (step_r != 0 || step_c != 0)) {
                int esq = er * 8 + ec;
                if (get_bit(all_pieces, esq)) {
                    int p1_type = b.get_piece_on_square(esq);
                    int p1_color = get_bit(b.occupancies[WHITE], esq) ? WHITE : BLACK;
                    ...
                    if (p1_color == us && p1_type == KING) {
                        int safe_dist = dist_to_falloff(er, ec);
                        bool has_p2 = false;
                        int p2_r = er + step_r;
                        int p2_c = ec + step_c;
                        if (p2_r >= 0 && p2_r < 8 && p2_c >= 0 && p2_c < 8) {
                            if (get_bit(b.occupancies[BOTH], p2_r * 8 + p2_c)) {
                                has_p2 = true;
                            }
                        }
                        ...
                        for (int step = 0; step < max_slide; step++) {
                            if (cur_r >= 0 && cur_r < 8 && cur_c >= 0 && cur_c < 8) {
                                if (get_bit(b.occupancies[BOTH], cur_r * 8 + cur_c)) {
                                    break;
                                }
                            }
                            slide_dist++;
                    ...
                    if (!is_invalid) {
                        ...
                        if (p2_r >= 0 && p2_r < 8 && p2_c >= 0 && p2_c < 8) {
                            ...
                            if (get_bit(all_pieces, p2_sq)) {
                                int p2_type = b.get_piece_on_square(p2_sq);
                                int p2_color = get_bit(b.occupancies[WHITE], p2_sq) ? WHITE : BLACK;
                                if (p2_color == us && p2_type == KING) {
                                    int safe_dist = dist_to_falloff(p2_r, p2_c);
                                    int slide_dist = 0;
                                    int cur_r = p2_r + step_r;
                                    int cur_c = p2_c + step_c;
    ...
    U64 pawns = (b.pieces[us][PAWN] | b.pieces[us][GOLDEN_PAWN] |
                 b.pieces[us][EPEE_PAWN] | b.pieces[us][PAWN_KNIFE] | 
    ...
    if (us == WHITE) {
        ...
        if (b.en_passant_sq != NO_SQ) {
            ...
            // EPEE PAWN GLOBAL EN PASSANT
            U64 epees = b.pieces[us][EPEE_PAWN] & ~paralyzed_squares;
            while (epees) {
                int epee_sq = pop_lsb(epees);
                add_pawn_move(epee_sq, b.en_passant_sq, EPEE_PAWN, true, b.get_piece_on_square(cap_sq), true);
            }
        }
    } else {
        // Single pushes
    ...
    if (us == WHITE) {
        bool white_king_on_e1 = get_bit(b.pieces[WHITE][KING], E1);
        if (white_king_on_e1) {
            int k_type = KING;
        ...
        bool black_king_on_e8 = get_bit(b.pieces[BLACK][KING], E8);
        if (black_king_on_e8) {
            int k_type = KING;
