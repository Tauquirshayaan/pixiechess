...
struct MoveList {
    ...
    int scores[512];
    bool is_losing_capture[512]; // SEE cache to prevent double-evaluation
    int count;

    MoveList() : count(0) {}
    
    inline void add(Move m) {
        if (count >= 512) return; // Prevent buffer overflow in extreme fuzzer scenarios
        scores[count] = 0;
        is_losing_capture[count] = false;
        moves[count++] = m;
    }
};
