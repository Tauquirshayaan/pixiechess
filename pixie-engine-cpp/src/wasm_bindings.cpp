...
const char* test_is_attacked(const char* pfen_cstr, int sq, int attacker_color) {
    ...
    U64 pawns = (b->pieces[ac][PAWN] | b->pieces[ac][GOLDEN_PAWN] | b->pieces[ac][EPEE_PAWN] | b->pieces[ac][PAWN_KNIFE] | b->pieces[ac][HERO_PAWN] | b->pieces[ac][WARP_JUMPER] | b->pieces[ac][WAR_AUTOMATON] | b->pieces[ac][SHRIKE]) & valid_attackers;
    if (ac == WHITE) {
    ...
    U64 knights = (b->pieces[ac][KNIGHT] | b->pieces[ac][ELECTROKNIGHT] | b->pieces[ac][BANKER] | b->pieces[ac][KNIGHTMARE] | b->pieces[ac][FISH_KNIGHT]) & valid_attackers;
    if (KNIGHT_ATTACKS[sq] & knights) reason = "true (knight)";
    
    U64 kings = b->pieces[ac][KING] & valid_attackers;
...
const char* test_pseudo_moves(const char* pfen_cstr) {
    ...
    while (basilisks) {
        ...
        paralyzed_squares |= get_sliding_attacks(sq, b->occupancies[BOTH], true, false);
    }
...
const char* debug_move_legality(const char* pfen_cstr, int f, int t) {
    ...
    for(int i=0; i<pseudo.count; i++) {
        if(pseudo.moves[i].from() == f && pseudo.moves[i].to() == t) {
            ...
            U64 king_bb = copy->pieces[us][KING];
            ss << "king_bb=" << king_bb << "\n";
            if (!king_bb) {
                ss << "No king found!\n";
            } else {
