...
export default function ChessBoard({ validMoves, threatenedSquares = [], onSquareClick, onDoubleClick, onDragStart, onDrop, onPieceHover }: Props) {
  ...
  useEffect(() => {
    ...
    setGlowActive(true);
    const tid = setTimeout(() => setGlowActive(false), 10000);
  ...
  for (const ob of offBoardPieces) {
    ...
    const isEnemyCapture = isValidMove && validMoves?.some(m => m.to[0] === r && m.to[1] === c && m.capture);
    floatingTiles.push(renderFloatingTile(r, c, ob.piece, !!isValidMove, !!isEnemyCapture));
  }
