...
export default function ChessSquare({
  ...
  onSquareClick, onDoubleClick, onDragStart, onDrop, onPieceHover, style
}: Props) {
  ...
  // console.log("Rendering ChessSquare without orange glow", { r, c, piece });
  ...
  const gsTargets       = piece?.pixie === 'GUNSLINGER' ? (piece.state?.gs_targets || []) : [];
  const isGunslingerReady = gsTargets.length > 0; // Glow immediately when it recognizes a threat
  ...
  return (
    ...
    >
      ...
      {/* ── Pending Freeze Warning (Icicle) (DISABLED) ── */}
      {/* 
      ...
      */}
      ...
      {/* ── Gunslinger Ready Glow ── */}
      {isGunslingerReady && (
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'rgba(249, 115, 22, 0.3)',
          boxShadow: 'inset 0 0 12px rgba(249, 115, 22, 0.8)',
          zIndex: 3,
          pointerEvents: 'none',
        }} />
      )}
      ...
      {piece && sym && (
        ...
        >
          <span style={{
            ...
            zIndex: 2,
            position: 'relative',
            color: (piece.pixie === 'HERO_PAWN') ? 'transparent' : (meta ? meta.color : (piece.id.startsWith('hordeling_') ? '#C084FC' : (piece.color === 'w' ? '#FFFDE7' : '#1A1A2E'))),
            backgroundImage: (piece.pixie === 'HERO_PAWN' ? 'linear-gradient(to bottom, #DC2626 50%, #38BDF8 50%)' : 'none'),
            WebkitBackgroundClip: (piece.pixie === 'HERO_PAWN') ? 'text' : undefined,
            WebkitTextFillColor: (piece.pixie === 'HERO_PAWN') ? 'transparent' : undefined,
          ...
          {piece?.pixie === 'PILGRIM' && pilgrimDist > 0 && (
            <div style={{
              position: 'absolute', top: -4, left: -4,
              background: pilgrimDist >= 15 ? '#EF4444' : '#3B82F6',
              color: '#FFF', fontSize: 10, fontWeight: 'bold',
              padding: '2px 4px', borderRadius: 4, zIndex: 6,
              boxShadow: '0 0 4px rgba(0,0,0,0.5)',
            }}>
          ...
          {isFrozen && (
            <div style={{
              ...
              pointerEvents: 'none',
              filter: 'drop-shadow(0 0 2px #000)',
            }} title="Frozen (Double-click to unfreeze)">
              ❄️
            </div>
          )}
        </motion.div>
