...
export default function GameScreen() {
  ...
  useEffect(() => {
    ...
    if (isCalculating) {
      ...
      const tick = (now: number) => {
        setCalcTimeMs(Math.floor(now - startTimestamp!));
              ...
              if (!move) {
                ...
                if (currentState.autoMove) {
                  setTimeout(() => {
                    ...
                    // Do not clear pending result so the engine output remains visible
                  }, 300);
                }
              }
              setCalculating(false);
  ...
  const applyResult = useCallback(() => {
    ...
    if (!res?.move || !res?.board) return;
    const currentState = useGameStore.getState();
    const tracker = new AbilityTrackerImpl();
    handleCaptureAnim(res.move, res.board);
    const { board: nb, effects, gameState: ngs } = applyMove(res.board, res.move, currentState.gameState, tracker);
    if (effects.includes('PIXIE_WIN')) {
      setWinMsg('✦ Golden Pawn promotes — GAME OVER!');
    } else if (effects.includes('KING_DESTROYED')) {
      setWinMsg('☠️ King Destroyed — GAME OVER!');
  ...
  }, [applyBoardMove, setLastMove, showToast, addCapture, setPendingResult]);
  ...
  const handleSquareClick = useCallback((r: number, c: number, isDrop = false) => {
    ...
    if (state.moveSel) {
      ...
      if (knightmareDrop) {
        const dropMove = validMoves.find(m => m.to[0] === r && m.to[1] === c && (m.drop === 'KNIGHTMARE' || m.obJump));
        if (dropMove) {
  ...
  const handleDoubleClick = useCallback((r: number, c: number) => {
    ...
    const turnColor = state.gameState.turn % 2 === 1 ? 'w' : 'b';
    if (piece.color !== turnColor) return;

    let executedMove: Move | null = null;

    const isFrozen = state.gameState.frozen.some(f => f.square[0] === r && f.square[1] === c);
    if (isFrozen) {
      if (isCheck(state.board, turnColor, state.gameState)) {
        showToast('Cannot unfreeze while in check!');
        return;
      }
      executedMove = { from: [r, c], to: [r, c], capture: false, unfreeze: true };
    } else if (piece.pixie === 'GUNSLINGER') {
      const moves = getLegalMoves(state.board, r, c, state.gameState, piece.state);
      const duelMove = moves.find(m => m.duel && m.to[0] === r && m.to[1] === c);
      if (duelMove) {
        executedMove = duelMove;
      }
    }
    ...
    const tracker = new AbilityTrackerImpl();
    handleCaptureAnim(executedMove, state.board);
    const { board: nb, gameState: ngs, effects } = applyMove(state.board, executedMove, state.gameState, tracker);
    applyBoardMove(nb, ngs);
    playSoundEffects(effects, !!executedMove.capture);
    setLastMove([r, c], [r, c]);
    
    if (executedMove.unfreeze) showToast('Piece unfrozen!');
    
    setMoveSel(null);
    setMoveHl([]);
    setValidMoves([]);
  }, [applyBoardMove, setLastMove, showToast, setMoveSel, setMoveHl, setValidMoves]);
