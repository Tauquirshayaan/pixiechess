...
export default function ResultPanel({ isCalculating, pendingResult, error, winMsg, evalStream, onApply, onSelectPv, autoMove, calcTimeMs }: Props) {
  if (isCalculating) {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {calcTimeMs !== undefined && (
            <div style={{ color: '#64748B', fontSize: 14, fontWeight: 600, fontFamily: "'Roboto Condensed', 'Inter', sans-serif" }}>
              ⏱️ {(calcTimeMs / 1000).toFixed(1)}s
            </div>
          )}
        </div>
  ...
  const handlePreviewAndSelect = (e: React.MouseEvent, previewMove: Move, pvScore: number) => {
    ...
    // 1. Instantly reset board and gameState to pre-calc state
    store.setBoard(pendingResult.board);
    
    // Restore the actual last move and gameState from history so the board looks exactly as it did before preview
    const historyEntry = store.stateHistory[store.historyIndex];
    if (historyEntry) {
      store.setLastMove(historyEntry.lastMoveFrom, historyEntry.lastMoveTo);
      store.setGameState(historyEntry.gameState);
    } else {
      store.setLastMove(null, null);
    }

    // 2. After a short delay, execute the move to show animation
    // Using 250ms to allow Framer Motion's spring animation to return the piece to its original square
    setTimeout(() => {
      // Use applyMove for the preview so captures and abilities are visually represented perfectly!
      const tracker = new AbilityTrackerImpl();
      const currentGameState = store.gameState;
      const { board: previewBoard, gameState: previewGameState } = applyMove(pendingResult.board!, previewMove, currentGameState, tracker);
      
      store.setBoard(previewBoard);
      store.setGameState(previewGameState);
      ...
      {pendingResult.multiPv && pendingResult.multiPv.length > 1 && (
        <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {pendingResult.multiPv.map((pv, i) => {
            ...
            if (JSON.stringify(pv.move) === JSON.stringify(move)) return null;
