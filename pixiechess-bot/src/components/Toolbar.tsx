...
export default function Toolbar() {
  ...
  const historyLength = useGameStore(s => s.stateHistory.length);
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <button onClick={undo} disabled={!canUndo} style={{ ...btnStyle(false), opacity: canUndo ? 1 : 0.5, cursor: canUndo ? 'pointer' : 'default' }}>◀ Back</button>
      <button onClick={redo} disabled={!canRedo} style={{ ...btnStyle(false), opacity: canRedo ? 1 : 0.5, cursor: canRedo ? 'pointer' : 'default' }}>Forward ▶</button>
