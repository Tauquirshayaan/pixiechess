...
export function applyMove(
  ...
  function onPieceDestroyed(destroyed: Piece): void {
    if (destroyed.pixie === 'HORDE_MOTHER' || destroyed.id.startsWith('hordeling_')) {
      // Clear from board
      for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
          const p = nb[r][c];
          if (p && p.color === destroyed.color && (p.pixie === 'HORDE_MOTHER' || p.id.startsWith('hordeling_'))) {
            nb[r][c] = null;
          }
        }
      }
      // Clear from offBoardPieces (limbo)
      ngs.offBoardPieces = ngs.offBoardPieces.filter(ob => !(ob.piece.color === destroyed.color && (ob.piece.pixie === 'HORDE_MOTHER' || ob.piece.id.startsWith('hordeling_'))));
      effects.push('HORDE_DEATH');
    }
    
    // Do not add ephemeral hordelings to deadPieces pool for resurrection
    if (!destroyed.id.startsWith('hordeling_')) {
  ...
  if (move.capture) {
    ...
    automatonsToMove.sort((a, b) => {
      ...
      // Sort front-to-back to prevent self-blocking
      if (a.p.color === 'w') {
         return a.r - b.r; // White moves to r=0, so lower r is FRONT
      } else {
         return b.r - a.r; // Black moves to r=7, so higher r is FRONT
      }
    });
