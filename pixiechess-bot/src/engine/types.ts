export type PixieType =
  // Pawns
  | 'GOLDEN_PAWN' | 'IRONPAWN' | 'EPEE_PAWN' | 'PAWN_KNIFE'
  | 'HERO_PAWN' | 'SHRIKE' | 'WARP_JUMPER' | 'WAR_AUTOMATON' | 'HORDELING'
  // Knights
  | 'ELECTROKNIGHT' | 'BANKER' | 'CAMEL' | 'KNIGHTMARE' | 'ANTI_VIOLENCE'
  | 'FISH_KNIGHT'
  // Bishops
...
export interface Move {
  ...
  unfreeze?: boolean;            // Used by frontend for explicit double-click unfreeze
}
...
export interface AbilityState {
  ...
  mutual_ply?: number;
  gs_targets?: Array<{ sq: [number, number]; ply: number }>;
