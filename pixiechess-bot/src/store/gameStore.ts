...
import { boardToPFEN } from '../engine/pfen';
...
interface GameStore {
  ...
  // ── History Navigation ──
  stateHistory: { board: Board; gameState: GameState; lastMoveFrom: [number, number] | null; lastMoveTo: [number, number] | null }[];
  historyIndex: number;
...
export const useGameStore = create<GameStore>()(
  ...
  undo: () => {
    set((state) => {
      if (state.historyIndex > 0) {
        ...
        const entry = state.stateHistory[newIndex];
        return {
  ...
  redo: () => {
    set((state) => {
      if (state.historyIndex < state.stateHistory.length - 1) {
        const newIndex = state.historyIndex + 1;
        const entry = state.stateHistory[newIndex];
        return {
          historyIndex: newIndex,
  ...
  placePiece: (r, c) => {
    set((state) => {
      ...
      if (sel.pixie) {
        ...
        if (target) {
          if (target.type !== baseType || target.color !== sel.color || target.pixie) {
        ...
        // 2. Maximum of 10 power pieces allowed per side manually
        let powerCount = 0;
        ...
        if (powerCount >= 10) {
          state.showToast(`Maximum of 10 power pieces allowed for ${sel.color === 'w' ? 'White' : 'Black'}!`);
          return state;
        }
      }
      ...
      // Reset game over and winner when board is manually modified
      const newGameState = { ...state.gameState, gameOver: false, winner: undefined };
      
      const newHistory = state.stateHistory.slice(0, state.historyIndex + 1);
      newHistory.push({
        board: newBoard,
  ...
  updateBoardStatus: () => {
    ...
    // ── Always use a fresh paralysis state for accurate check detection ──
    const freshParalysisW = calcBasiliskParalysis(board, 'w');
    const freshGameStateW = { ...gameState, paralyzed: { ...gameState.paralyzed, w: freshParalysisW } };
    const wInCheck = isCheck(board, 'w', freshGameStateW);
    const wLegalMoves = wInCheck ? getAllMovesForColor(board, 'w', freshGameStateW) : [];

    const freshParalysisB = calcBasiliskParalysis(board, 'b');
    const freshGameStateB = { ...gameState, paralyzed: { ...gameState.paralyzed, b: freshParalysisB } };
    const bInCheck = isCheck(board, 'b', freshGameStateB);
    const bLegalMoves = bInCheck ? getAllMovesForColor(board, 'b', freshGameStateB) : [];

    // Completely resilient Checkmate detection regardless of Sandbox/Preview turn-desyncs
    if (wInCheck && wLegalMoves.length === 0) {
      set({ boardStatus: 'checkmate', statusWinner: 'b' });
      return;
    }
    if (bInCheck && bLegalMoves.length === 0) {
      set({ boardStatus: 'checkmate', statusWinner: 'w' });
      return;
    }

    // Check stalemate for the active player
    const activeInCheck = colorToMove === 'w' ? wInCheck : bInCheck;
    const activeLegalMoves = getAllMovesForColor(board, colorToMove, colorToMove === 'w' ? freshGameStateW : freshGameStateB);
    
    if (!activeInCheck && activeLegalMoves.length === 0) {
      set({ boardStatus: 'game_over', statusWinner: null }); // Stalemate
      return;
    }

    const inCheck = activeInCheck;

    // ── Threefold Repetition Draw (checked only if game hasn't ended) ──
    const { pfenHistory } = get();
    if (pfenHistory.length >= 1) {
      const currentPfen = boardToPFEN(board, colorToMove, gameState);
      const count = pfenHistory.filter(p => p === currentPfen).length;
      if (count >= 3) {
        set({ boardStatus: 'game_over', statusWinner: null }); // Draw by repetition
        return;
      }
    }
  ...
  applyBoardMove: (newBoard, newGs) => {
    const colorToMove = newGs.turn % 2 === 1 ? 'w' : 'b';
    const newPfen = boardToPFEN(newBoard, colorToMove, newGs);
    set((state) => {
      const newHistory = state.stateHistory.slice(0, state.historyIndex + 1);
      newHistory.push({
        board: newBoard,
        gameState: newGs,
        lastMoveFrom: newGs.lastMove?.from || null,
        lastMoveTo: newGs.lastMove?.to || null
      });
      return {
        board: newBoard,
        gameState: newGs,
        moveHl: [],
        moveSel: null,
        pfenHistory: [...state.pfenHistory, newPfen],
        stateHistory: newHistory,
        historyIndex: newHistory.length - 1,
        lastMoveFrom: newGs.lastMove?.from || null,
        lastMoveTo: newGs.lastMove?.to || null
