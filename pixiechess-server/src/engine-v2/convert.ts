...
const MIGRATED_PIXIES = new Set([
  ...
  'CARDINAL', 'ELECTROKNIGHT', 'DJINN', 'HERO_PAWN',
  'BANKER', 'PILGRIM', 'ARISTOCRAT', 'DANCER', 'FISH_KNIGHT',
      ...
      if (piece.pixie) {
        if (MIGRATED_PIXIES.has(piece.pixie)) {
          ...
          if (piece.color === 'w') {
            state.whitePixies |= b;
            state.whiteAll |= b;
            if (piece.pixie === 'PHASE_ROOK' || piece.pixie === 'SUMOROOK') state.whiteCastlingRooks |= b;
          } else {
            state.blackPixies |= b;
            state.blackAll |= b;
            if (piece.pixie === 'PHASE_ROOK' || piece.pixie === 'SUMOROOK') state.blackCastlingRooks |= b;
          }
        } else {
