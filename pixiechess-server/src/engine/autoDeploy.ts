...
function placementBonus(baseType: string, pixie: string, r: number, c: number, color: 'w' | 'b', board: Board): number {
  ...
  switch (baseType) {
    case 'P': {
      ...
      const isPowerPawn = pixie !== 'NONE' && pixie !== undefined;
      
      if (isPowerPawn) {
        // DO NOT deploy power pawns in the center (d and e files)
        if (c === 3 || c === 4) {
           bonus -= 10.0; // Massive penalty forces them to the flanks
        } else {
           bonus += 1.0; // Small bonus for being on the flanks (a,b,c,f,g,h)
        }
      } else {
        // Standard pawns strongly prefer the center
        if (c === 3 || c === 4) bonus += 5.0;
        else if (c === 2 || c === 5) bonus += 2.0;
      }
      ...
      // PAWN_KNIFE: C and F files
      if (pixie === 'PAWN_KNIFE') {
        if (c === 2 || c === 5) bonus += 2.5;
      }

      // EPEE_PAWN, HERO_PAWN, IRONPAWN previously preferred the center, but the user explicitly requested 
      // ALL power pawns be placed on flanks. The -10.0 penalty above guarantees they stay out of d and e.
      break;
    }
    case 'N': {
