...
app.post('/api/calculate', async (req, res) => {
  try {
    activeCalculations++;
    const { board, gameState, color, depth, timeLimitMs, multiPv } = req.body;
    ...
    const [result] = await Promise.all([
      findBestMoveUCI(board, color, depth || 7, gameState, timeLimitMs, engineThreads, multiPv || 1),
...
app.post('/api/calculate-stream', async (req, res) => {
  try {
    ...
    const bestMove = await findBestMoveUCI(
      board, color, depth || 7, gameState, timeLimitMs, engineThreads, multiPv || 1, onInfo
    );
