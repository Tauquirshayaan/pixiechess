...
// Automatically use all available logical cores EXCEPT 2 (leaving 1 full physical core safe for OS and Node.js)
const MAX_VCPUS = Math.max(2, os.cpus().length - 2);
...
app.post('/api/calculate', async (req, res) => {
  try {
    ...
    const [result] = await Promise.all([
      findBestMoveUCI(board, color, depth || 15, gameState, timeLimitMs, engineThreads, multiPv || 1, undefined, pfenHistory || []),
      new Promise(resolve => setTimeout(resolve, minWait))
    ]);
    
    // Automatically harvest training data
    try {
      if (result.move && result.score !== undefined) {
        const pfen = boardToPFEN(board, color, gameState);
        const dataLogPath = path.resolve(__dirname, '../../../training_data.jsonl');
        
        // Log perspective: We save the score from the engine's perspective (the side to move)
        // However, NNUE expects the score from White's perspective, so we convert it.
        ...
        fs.appendFileSync(dataLogPath, JSON.stringify(dataObj) + '\n');
      }
    } catch (e) {
      console.error('[DataHarvest] Failed to save training data:', e);
    }
...
app.post('/api/calculate-stream', async (req, res) => {
  try {
    ...
    const bestMove = await findBestMoveUCI(
      board, color, depth || 15, gameState, timeLimitMs, engineThreads, multiPv || 1, onInfo, pfenHistory || []
    );
